const byId = (id) => document.getElementById(id);

export const els = {
  promptInput: byId('promptInput'),
  promptMeta: byId('promptMeta'),
  promptRolloutMeta: byId('promptRolloutMeta'),
  tokenList: byId('tokenList'),
  nextMeta: byId('nextMeta'),
  nextTableBody: byId('nextTable').querySelector('tbody'),
  topK: byId('topK'),
  tokenizeBtn: byId('tokenizeBtn'),
  decodeBtn: byId('decodeBtn'),
  nextBtn: byId('nextBtn'),
  checkpointSelect: byId('checkpointSelect'),
  checkpointMeta: byId('checkpointMeta'),
  runMeta: byId('runMeta'),
  loadCheckpointBtn: byId('loadCheckpoint'),
  sessionRunSelect: byId('sessionRunSelect'),
  loadSessionRunBtn: byId('loadSessionRun'),
  sessionRunMeta: byId('sessionRunMeta'),
  runTokenizerReportWrap: byId('runTokenizerReport'),
  runTokenizerReportRefreshBtn: byId('runTokenizerReportRefresh'),
  runTokenizerReportMeta: byId('runTokenizerReportMeta'),
  runTokenizerReportTableBody: byId('runTokenizerReportTable').querySelector('tbody'),
  runInspectorTabSelect: byId('runInspectorTabSelect'),
  runLogsSelect: byId('runLogsSelect'),
  inspectMeta: byId('inspectMeta'),
  layerTopkTableBody: byId('layerTopkTable').querySelector('tbody'),
  inspectTopK: byId('inspectTopK'),
  inspectBtn: byId('inspectBtn'),
  inspectSampleSteps: byId('inspectSampleSteps'),
  inspectTemperature: byId('inspectTemperature'),
  inspectTopP: byId('inspectTopP'),
  inspectTopKSample: byId('inspectTopKSample'),
  inspectRepetitionPenalty: byId('inspectRepetitionPenalty'),
  inspectGenerateBtn: byId('inspectGenerateBtn'),
  inspectorTabSelect: byId('inspectorTabSelect'),
  inspectorActiveToken: byId('inspectorActiveToken'),
  runSetupMeta: byId('runSetupMeta'),
  runCreateBtn: byId('runCreate'),
  runName: byId('runName'),
  runsMeta: byId('runsMeta'),
  refreshRunsBtn: byId('refreshRuns'),
  runsSearch: byId('runsSearch'),
  runsFilterStatus: byId('runsFilterStatus'),
  runsActiveList: byId('runsActiveList'),
  runsHistoryList: byId('runsHistoryList'),
  runDetailMeta: byId('runDetailMeta'),
  runDatasetName: byId('runDatasetName'),
  runTokenizerName: byId('runTokenizerName'),
  runTrainingConfigName: byId('runTrainingConfigName'),
  runDetails: byId('runDetails'),
  runLossChart: byId('runLossChart'),
  runLossMeta: byId('runLossMeta'),
  runLogs: byId('runLogs'),
  runLoadStatus: byId('runLoadStatus'),
  runProgressMeta: byId('runProgressMeta'),
  runProgressLine: byId('runProgressLine'),

  datasetPath: byId('datasetPath'),
  datasetRefreshBtn: byId('datasetRefresh'),
  datasetEntries: byId('datasetEntries'),
  datasetSelections: byId('datasetSelections'),
  datasetMeta: byId('datasetMeta'),
  datasetName: byId('datasetName'),
  datasetEnableHashes: byId('datasetEnableHashes'),
  datasetCreateBtn: byId('datasetCreate'),
  datasetCreateMeta: byId('datasetCreateMeta'),
  datasetManifestList: byId('datasetManifestList'),
  datasetRefreshManifestsBtn: byId('datasetRefreshManifests'),
  datasetPreviewSelect: byId('datasetPreviewSelect'),
  datasetManifestPreview: byId('datasetManifestPreview'),
  datasetManifestMeta: byId('datasetManifestMeta'),
  datasetManifestFiles: byId('datasetManifestFiles'),

  datasetSelect: byId('datasetSelect'),
  refreshDatasetsBtn: byId('refreshDatasets'),

  runTokenizerVocabSelect: byId('runTokenizerVocabSelect'),
  refreshTokenizerVocabsBtn: byId('refreshTokenizerVocabs'),

  trainingConfigSelect: byId('trainingConfigSelect'),
  refreshTrainingConfigsBtn: byId('refreshTrainingConfigs'),

  runInitCheckpointSelect: byId('runInitCheckpointSelect'),
  refreshInitCheckpointsBtn: byId('refreshInitCheckpoints'),
  runInitModeSelect: byId('runInitModeSelect'),
  runInitCheckpointMeta: byId('runInitCheckpointMeta'),
  runOverrideLearningRate: byId('runOverrideLearningRate'),
  runOverrideMaxIters: byId('runOverrideMaxIters'),
  runOverrideBatchSize: byId('runOverrideBatchSize'),
  runOverrideEvalInterval: byId('runOverrideEvalInterval'),
  runOverrideWarmupIters: byId('runOverrideWarmupIters'),
  runOverrideSeed: byId('runOverrideSeed'),
  runOverrideDevice: byId('runOverrideDevice'),

  tokenizerConfigList: byId('tokenizerConfigList'),
  tokenizerConfigEditor: byId('tokenizerConfigEditor'),
  tokenizerConfigMeta: byId('tokenizerConfigMeta'),
  tokenizerConfigNewBtn: byId('tokenizerConfigNew'),
  tokenizerConfigSaveBtn: byId('tokenizerConfigSave'),
  tokenizerConfigDuplicateBtn: byId('tokenizerConfigDuplicate'),
  tokenizerConfigSearch: byId('tokenizerConfigSearch'),
  tokenizerConfigSort: byId('tokenizerConfigSort'),
  tokenizerConfigTabSelect: byId('tokenizerConfigTabSelect'),
  tokenizerConfigSummary: byId('tokenizerConfigSummary'),
  tokenizerConfigDefaults: byId('tokenizerConfigDefaults'),

  tokenizerVocabList: byId('tokenizerVocabList'),
  tokenizerVocabRefreshBtn: byId('tokenizerVocabRefresh'),
  tokenizerVocabRefreshListBtn: byId('tokenizerVocabRefreshList'),
  tokenizerVocabConfigSelect: byId('tokenizerVocabConfigSelect'),
  tokenizerVocabRefreshConfigsBtn: byId('tokenizerVocabRefreshConfigs'),
  tokenizerVocabDatasetSelect: byId('tokenizerVocabDatasetSelect'),
  tokenizerVocabRefreshDatasetsBtn: byId('tokenizerVocabRefreshDatasets'),
  tokenizerVocabName: byId('tokenizerVocabName'),
  tokenizerVocabCreateBtn: byId('tokenizerVocabCreate'),
  tokenizerVocabMeta: byId('tokenizerVocabMeta'),
  tokenizerVocabProgressMeta: byId('tokenizerVocabProgressMeta'),
  tokenizerVocabProgressLine: byId('tokenizerVocabProgressLine'),
  tokenizerVocabDetails: byId('tokenizerVocabDetails'),
  tokenizerVocabDetailsMeta: byId('tokenizerVocabDetailsMeta'),
  tokenizerVocabTabSelect: byId('tokenizerVocabTabSelect'),
  tokenizerVocabConfig: byId('tokenizerVocabConfig'),
  tokenizerVocabConfigMeta: byId('tokenizerVocabConfigMeta'),
  tokenizerVocabTokenSearch: byId('tokenizerVocabTokenSearch'),
  tokenizerVocabTokenLimit: byId('tokenizerVocabTokenLimit'),
  tokenizerVocabTokenRefreshBtn: byId('tokenizerVocabTokenRefresh'),
  tokenizerVocabTokenMeta: byId('tokenizerVocabTokenMeta'),
  tokenizerVocabTokens: byId('tokenizerVocabTokens'),

  trainingConfigList: byId('trainingConfigList'),
  trainingConfigEditor: byId('trainingConfigEditor'),
  trainingConfigMeta: byId('trainingConfigMeta'),
  trainingConfigNewBtn: byId('trainingConfigNew'),
  trainingConfigSaveBtn: byId('trainingConfigSave'),
  trainingConfigDuplicateBtn: byId('trainingConfigDuplicate'),
  trainingConfigSearch: byId('trainingConfigSearch'),
  trainingConfigSort: byId('trainingConfigSort'),
  trainingConfigTabSelect: byId('trainingConfigTabSelect'),
  trainingConfigSummary: byId('trainingConfigSummary'),
  trainingConfigHyperparams: byId('trainingConfigHyperparams'),

  chatRunMeta: byId('chatRunMeta'),
  chatSessionRunSelect: byId('chatSessionRunSelect'),
  chatLoadSessionRunBtn: byId('chatLoadSessionRun'),
  chatSessionRunMeta: byId('chatSessionRunMeta'),
  chatCheckpointSelect: byId('chatCheckpointSelect'),
  chatLoadCheckpointBtn: byId('chatLoadCheckpoint'),
  chatCheckpointMeta: byId('chatCheckpointMeta'),
  chatMaxTokens: byId('chatMaxTokens'),
  chatTemperature: byId('chatTemperature'),
  chatTopP: byId('chatTopP'),
  chatTopK: byId('chatTopK'),
  chatRepetitionPenalty: byId('chatRepetitionPenalty'),
  chatInput: byId('chatInput'),
  chatSendBtn: byId('chatSend'),
  chatClearBtn: byId('chatClear'),
  chatStatus: byId('chatStatus'),
  chatTranscriptMeta: byId('chatTranscriptMeta'),
  chatDebugToggle: byId('chatDebugToggle'),
  chatMessages: byId('chatMessages'),
  chatTranscript: byId('chatTranscript'),
};
